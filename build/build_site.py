...
(function(){
  ...
  function shootStar(myGen){
    ...
    const x0=W*.0+Math.random()*W*.25, y0=H*.7+Math.random()*H*.3;
    const x2=W*.65+Math.random()*W*.35, y2=Math.random()*H*.3;
    // control point pulls the arc outward (up and right)
    const cx=x0+W*.05, cy=y2-H*.15;
    ...
    function frame(now){
      ...
      else{ ctx.clearRect(0,0,W,H); setTimeout(()=>shootStar(myGen),2500+Math.random()*2500); }
    }
